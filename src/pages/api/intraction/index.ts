...
async function handleGetMethodByPostId(
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Get user intractions by postId succesfully",
      status: 200,
      data: response,
    });
  } catch (err) {
...
async function handleGetMethodByUserIdAndPostId(
  req: NextApiRequest,
  res: NextApiResponse<ResponseType>,
) {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Get user intractions by userId succesfully",
      status: 200,
      data: response,
    });
  } catch (err) {
...
async function handleGetMethodDefault(
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Get users intractions succesfully",
      status: 200,
      data: response,
    });
  } catch (err) {
...
async function handlePostMethod(
  ...
  try {
    const response = await prisma.intractions.create({
      data: {
        userId,
        postId: parseInt(String(postId)),
        liked,
    ...
    return res.status(201).json({
      success: true,
      message: "Create user intractions succesfully",
      status: 201,
      data: response,
    });
  } catch (err) {
...
async function handlePutMethod(
  ...
  const { liked } = req.body;
  try {
    const response = await prisma.intractions.update({
      where: {
    ...
    return res.status(200).json({
      success: true,
      message: "Update user intractions succesfully",
      status: 200,
      data: response,
    });
  } catch (err) {
...
async function handleDeleteMethod(
  ...
  const postId = req.query.postId as string;
  try {
    const response = await prisma.intractions.delete({
      where: {
        id: parseInt(String(id)),
        userId,
        postId: parseInt(String(postId)),
    ...
    return res.status(200).json({
      success: true,
      message: "Delete user intractions succesfully",
      status: 200,
      data: response,
    });
  } catch (err) {
